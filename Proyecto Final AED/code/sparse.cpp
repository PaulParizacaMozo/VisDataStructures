...
public:
  ...
  void dibujar(){
        ...
        archivo<<"digraph G{\n";
        if (maximus == minimus) {
          archivo << "\t\tIGUAL[fontcolor=black, label=\"" << maximus << "\",fillcolor=springgreen];\n";
        } else {
          archivo << "\t\tMAXIMO[fontcolor=black, label=\"" << maximus << "\",fillcolor=orange];\n";
          archivo << "\t\tMINIMO[fontcolor=white, label=\"" << minimus << "\",fillcolor=purple];\n";
        }
        archivo<<"\tnode[shape=octagon style=filled]\n\tedge[color= blue]\n";
        archivo<<"\tsubgraph M_Espar{\n";
        archivo<<"\t\tlabel = \"MATRIZ ESPARZA\"\n";
        archivo<<"\t\traiz[label = \"Union\",group=0, fontcolor=white,fillcolor=\"brown:blue\",gradientangle=\"300\"];\n";
