...
class Binomial_Heap{
  ...
  private://funciones con la que no puede interactuar el usuario
    ...
    Iterator find_max(){//encontrar al máximo
      ...
      Iterator Maxinum=it; //guardamos el mayor en otro iterator:
...
*/
      ...
      for(; itheads!=m_pHeads.end(); itheads++){// Recorre heads y imprime las cabezas
        ...
        while(!cola.empty()){
          ...
          for (; itsons!=it->m_Sons.end(); itsons++ ){
            if((*itsons)->m_visit == false){ // Si aun no fue visitado lo agregamos a la cola
              (*itsons)->m_visit == true;
              cola.push(*itsons);
            }
            c++;
    ...
    void Draw(){
      ...
      limpiar.open("BinoHeap.dot",ios::out); limpiar<<""; limpiar.close();
