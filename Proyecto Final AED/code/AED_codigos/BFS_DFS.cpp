...
template<class V, class A>
class FindVertexOrigenEnEdges{
  private:
    V m_dato;
  public:
    FindVertexOrigenEnEdges(const V& vertice){
      this->m_dato = vertice;
    }
    bool operator() (const Edge<V,A>& e1){
      return  m_dato==e1.m_pVertexOrigen->m_Dato;
    }
}; 
...
bool hayCiclo(map <V,V>& conectados, Edge<V,A>& e){
  ...
  if(find_set<V,A>(conectados,e.m_pVertexOrigen->m_Dato) != find_set<V,A>(conectados,e.m_pVertex->m_Dato)){ 
    union_sets<V,A>(conectados,e.m_pVertexOrigen->m_Dato,e.m_pVertex->m_Dato);
    return false;
  }
  return true;
}
...
void Kruskal( Graph<V,A> G ){
  G.Reset_Graph();
  vector < Edge<V,A> > MST;
  vector < Edge<V,A> > vectorEdges;
  map < V, V > conectados;

  for (auto it=G.m_Graph.begin(); it!= G.m_Graph.end(); it++) {
    conectados.insert( pair<V,V>(it->m_Dato, it->m_Dato) ); // Inicializamos los fathers de los nodos
  }
  ...
  while (MST.size()+1 < G.m_Graph.size() || !pq_minheap.empty()){
    e = pq_minheap.top();
    pq_minheap.pop();
    if(!hayCiclo(conectados,e)){ // Si la nueva arista no forma un ciclo con las demas, insertamos al MST
      MST.push_back(e);
    }
  }
  /*cout<<"\n----- Fathers -----"<<endl;
  ...
  for (auto it=MST.begin(); it!=MST.end(); it++){
    cout<<"["<<it->m_pVertexOrigen->m_Dato<<" -- "<<it->m_Dato<<" -- "<<it->m_pVertex->m_Dato<<"]"<<endl;
  }
}
