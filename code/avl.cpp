...
class Avl {  //árbol AVL
  ...
  public:
    ...
    NodoAVL<T>* find_nodo(T d){   //Similar al Binario Trees
      NodoAVL<T>* pTmp = m_pRoot;
    ...
    void Draw(){
      ...
      system("dot -Tpdf archivoAvl.dot -o archivoAvl.pdf");
      system("zathura archivoAvl.pdf &");
    }

    void Find_Draw_AVL(T d){
      if(!m_pRoot) return;
      ofstream limpiar, archivo;
      //ios::in -> al abrir escribe desde 0 el archivo
      //ios::app -> abrir archivo de modo en que puede añadir datos donde se quedo el cursor
      ...
      /*ENCONTRAR*/
      if(find_element(d)){
        NodoAVL<T>* nodo = find_nodo(d);
        archivo<<"\t\""<<nodo->m_Dato<<" | "<<nodo->m_FE<<"\" [fillcolor = \"#EEE111\"]\n";
      }
